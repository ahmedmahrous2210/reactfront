...
class CreditPointShareTranLogs extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loader:false,
  ...
  _fetchCreditPointShareLogs(pageNum) {
    apiService
      .creditShareTranLogs({"isValid":true, "channelId":"MASAPLAYER", "module":"MASA", "requestId":"IBOPLAYERAPP2", "userId":loggegdInData.id, "groupId": loggegdInData.roleId})
      .then((userListData) => {
        ...
        if (
          ...
          this.setState({
            usersData: userListData.data.ResellerCreditShareLog,
          });
        } else {
          this.setState({
            showAlert: true,
  ...
  componentDidMount() {
    this._fetchCreditPointShareLogs(0);
  }
  ...
  userTable = (user, index) => {
    return({
            ...
            creditorEmail: user.creditor_detail.email ? user.creditor_detail.email:  null,
            debitorEmail: user.debitor_detail.email ? user.debitor_detail.email : null,
            createdBy: user.creator.email,
...
export default CreditPointShareTranLogs;
