...
class Widget04 extends Component {
  render() {
    ...
    const getStyle = (iconColor) => {
      return iconColor === 'info' ? {color:'#63c2de'} :
      iconColor === 'success' ? {color:'#4dbd74'} :
      iconColor === 'primary' ? {color:'#20a8d8'} :
      iconColor === 'danger' ? {color:'#f86c6b'} :
      iconColor === 'process' ? {color:'#9ACD32'} :
    {color:''}
    }
      ...
      <Card className={classes} {...attributes}>
        <CardBody>
          <div className="h1  text-right mb-2">
            <i className={card.icon} style={getStyle(progress.color)}></i>
          </div>
