...
class MessageList extends Component {
  ...
  messageUpdate(appUpdate, index) {
    return ({
      ...
      start_time: formatDateTime(appUpdate.start_time),
      description: appUpdate.description,
      end_time: formatDateTime(appUpdate.end_time)
    })
  }
  ...
  _fetchappUpdateList = (pageNum) => {
    apiService.messageList(pageNum).then((appUpdateListdata) => {

      if (typeof appUpdateListdata.status != 'undefined' && appUpdateListdata.status) {

        this.setState({
          appUpdateData: appUpdateListdata.data.IBOMessage,
        });
      } else {
        this.setState({
          showAlert: true,
