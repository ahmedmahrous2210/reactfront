...
class CreditPaymentOrders extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loader: false,
    ...
    orders(order, index) {
        ...
        return ({
            ...
            creditPoint: order.credit_points,
            amount: order.amount,
            createdAt: formatDateTime(order.created_at),
            action: <>
                
                {order.status === '2' ?
                    <Link to="#" title="Reopen order" onClick={() => this.changeStatus(order.id, 3)}>
                        <i className="fa fa-unlock"></i> &nbsp;|&nbsp;
                    </Link>
                    : null}
                {order.status === '1' ?
                    <Link to="#" title="Withdraw order" onClick={() => this.changeStatus(order.id, 0)}>
    ...
    _fetchTickets = () => {
        ...
        apiService.myOrders(formData).then((myOrders) => {

            if (typeof myOrders.status != 'undefined' && myOrders.status) {

                this.setState({
                    myOrders: myOrders.data.MyOrders,
                });
            } else {
                this.setState({
                    showAlert: true,
    ...
    render() {
        ...
        const myOrders = this.state.myOrders.filter((order) => order.id);
        const data = {
            columns: [
                {
                    label: 'Order Id',
            ...
            rows: myOrders.map((order, index) => this.orders(order, index))
        };
            ...
            <div className='animated fadeIn'>
                ...
                <Row>
                    <Col xl={12}>
                        <Card>
                            <CardHeader>
                                <i className='fa fa-align-justify' /> My Orders {' '}
...
export default CreditPaymentOrders;