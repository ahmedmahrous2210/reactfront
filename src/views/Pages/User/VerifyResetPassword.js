...
export default class VerifyResetPassword extends Component {
    constructor(props) {
        super(props);
        let query = new URLSearchParams(this.props.location.search);
		let token = query.get('token') || 0;
        this.state = {
    ...
    componentDidMount(){
        let formData = {token:this.state.token}
        this.setState({ loader: true, showAlert: false }, () => {
    
            apiService
              .verifyTokenPassword(formData)
            ...
            .then((addUserResponse) => {
              if (
                ...
                this.setState({
                  ...
                  showAlert: true,
                  alertType: "success",
                  alertBody: "Password Updated Successfully.",
                });
              } else {
                this.setState({
                  loader: false,
                  showAlert: true,
                  alertType: "danger",
                  alertBody: "Failed to update the password."
                });
              }
            })
            .catch((error) => {
        ...
        return (
            ...
            <Row>
                       <Col xl="12">
                        ...
                        {min:6, message:"Password should be minimum of 6 digit."}
                        ]}>
                        <Input.Password />
                    </Form.Item>
